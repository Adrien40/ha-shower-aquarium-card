...
class AquariumShowerCard extends LitElement {
  ...
  _getCanvasHeight() {
    return computeCanvasHeight(this._config);
  }

  _updateCachedMetrics() {
    if (!this._hass || !this._config) return;
    const metrics = computeCachedMetrics(this._hass, this._config);
  ...
  _updatePhysics(timestamp) {
    ...
    const { tankTop, tankBottom, waterSurfaceY, waterRatio, isDead, isBoiling, speedMultiplier } = computeTankState({
      config: this._config,
      metrics,
    ...
    const deathStep = (deltaMs || 16.66) / 4500;
    const themeKey = this._config.theme || "freshwater";
    ...
    if (this._ancistrus) {
      if (isDead) {
        this._ancistrus.deathProgress = Math.min(
          1.0,
          (this._ancistrus.deathProgress || 0) + deathStep
        );
        ...
        if (this._ancistrus.state === "moving") {
          ...
          const step = Math.sign(dy) * Math.min(Math.abs(dy), 0.7 * userSpeed * delta);
    ...
    if (this._shrimp) {
      if (isDead) {
        this._shrimp.deathProgress = Math.min(
          1.0,
          (this._shrimp.deathProgress || 0) + deathStep
        );
        stateChanged = true;
      } else {
    ...
    if (this._crab) {
      if (isDead) {
        this._crab.deathProgress = Math.min(
          1.0,
          (this._crab.deathProgress || 0) + deathStep
        );
        stateChanged = true;
      } else {
  ...
  _renderAncistrus(isDead) {
    ...
    return svg`
      <g transform="translate(${anc.x}, ${anc.y}) scale(1.5,${isDead ? -1.5 : 1.5})">
  ...
  _renderStatusPanel(
    ...
    const { tempFraction, tempColor, volFraction, volColor } = computeGaugeState({
      currentTemp,
      currentVolume,
      targetBudget,
      deadlyTemp,
...
const existingIndex = window.customCards.findIndex((c) => c.type === "shower-aquarium-card");
